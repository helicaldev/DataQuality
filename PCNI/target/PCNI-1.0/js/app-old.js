...
$(document).ready(function() {
  ...
  treeTable = function() {
    var list, makeTable;
    list = $("#saved-reports-list");
    makeTable = function(files, iteration) {
      var arrow, checkbox, column, date, ext, file, i, key, link1, link2, row, span, value, _i, _j, _k, _len;
      if (iteration == null) {
        iteration = 0;
      }
      for (_i = 0, _len = files.length; _i < _len; _i++) {
        ...
        if (file.type === 'folder') {
          ...
          ext = file.resultfile ? file.resultfile.split('.').pop() : '-';
          date = moment(file.rundate, 'YYYY-MM-DD HH:mm:ss.SSS').format('MM/DD/YYYY hh:mm A');
          row.append(column.clone().append(link2)).append(column.clone().append(ext)).append(column.clone().append(date));
          list.append(row);
        }
      }
    };
