...
(function(window, document, $, _, Backbone, Dashboard) {
  ...
  Dashboard.Templates.chart = '<div class="chart" id="{{id}}"></div> <script>{{script}}</script>';
  ...
  chart = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      this.model.update = function() {
        ...
        alert("Caled");
        data = window.DashboardGlobals.chartData;
        console.log(data+"  JOSN Stringify: "+JSON.stringify(window.DashboardGlobals.chartData));
        request = $.ajax({
          type: 'POST',
